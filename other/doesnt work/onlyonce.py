from helper import parsecoords
import numpy as np
...
class AirfoilProfile:
    def __init__(self, panels, name, t=1, vortex=True):
        self.panels = panels
        self.name = name
...
class Panel:
    ...
    def __init__(self, xm, ym, theta, length):

        self.xm = xm  # center of panel on x axis
        self.ym = ym  # center of panel on y axis
        self.length = length  # panel length

        self.theta = theta  # angle of panel
...
xm, ym, theta, length = [], [], [], []
for i in range(n):
    xm.append((x[n-i] + x[n-i-1]) / 2) # center of panel on x axis
    ym.append((y[n-i] + y[n-i-1]) / 2) # center of panel on x axis

    length.append(np.sqrt((x[n-i-1] - x[n-i]) ** 2 + (y[n-i-1] - y[n-i]) ** 2))  # panel length

    theta.append(atan2(y[n-i-1] - y[n-i], x[n-i-1] - x[n-i]))  # angle of panel

panels = []
for i in range(n):
    panels.append(Panel(xm[i], ym[i], theta[i], length[i]))
panels = np.array(panels)

profile = AirfoilProfile(panels, "yo", t=t, vortex=True)
profile.solve()
print(profile)