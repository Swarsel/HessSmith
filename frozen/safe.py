...
class AirfoilProfile:
    ...
    def plot(self, height=3, width=6, sort=False):
        ...
        #ax1.grid()
        ax1.plot(xu, yu, color='k', linestyle='-', linewidth=1,)
        ax1.plot(xl, yl, color='k', linestyle='-', linewidth=1,)
        ax1.set(xlabel='x', ylabel='y')
        #ax1.set_title("Kutta-Bedingung")
        ...
        for panel in self.panels:  # Loop over all panels
            ...
            Y[1] = panel.ym + panel.length * np.sin(panel.delta)  # Panel ending Y point
            if (count == 0):  # For first panel
                X[1] = panel.xm + panel.length * 500 * np.cos(panel.delta + np.pi/2)
                Y[1] = panel.ym + panel.length * 500 * np.sin(panel.delta + np.pi/2)
                ax1.plot(X, Y, 'g:', label='$v_0$')  # Plot the first panel normal vector
            elif (count == len(self.panels)-1):  # For second panel
                X[1] = panel.xm + panel.length * 500 * np.cos(panel.delta + 3*np.pi / 2)
                Y[1] = panel.ym + panel.length * 500 * np.sin(panel.delta - np.pi / 2)
                ax1.plot(X, Y, 'g-', label='$v_{n-1}$')  # Plot the second panel normal vector
            #else:  # For every other panel
             #   ax1.plot(X, Y, 'k')
            count += 1
        ax1.legend(loc="lower right", prop={'size': 8})
        ax1.text(0.9, 0.02, r'$|v_1| = |v_{n-1}|$', fontsize=10)
