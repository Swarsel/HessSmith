from computations import compute_xi, compute_eta, compute_J, compute_I, compute_At, compute_An, system_matrix, \
    compute_M, compute_inhomogenity
import sympy as sym
...
class AirfoilProfile:
    def __init__(self, panels, name, vortex=True):
        ...
        self.vortex = vortex
        self.U = sum([panel.length for panel in panels])
        self.xi = compute_xi(self.len, panels)
        self.eta = compute_eta(self.len, panels)
        ...
        if self.vortex:
            self.M = system_matrix(self.An, self.At)
        else:
            self.M = compute_M(self.len, self.An)
