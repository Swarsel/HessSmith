...
Die Daten der Profile wurden der UIUC Airfoil Data Site entnommen und lagen dabei überwiegend im Selig- oder Lednicer-Format vor, \cite{uiuc}.\footnote{Eine Gegenüberstellung der beiden Formate ist in \nameref{appendix:a} zu finden.}
\\
...
\end{align*} 
für die Anströmgeschwindigkeit $V_{\infty} = 1$ und den Anströmwinkel $\alpha =5^{\circ}$ berechnet und sind in Form eines Strömungsgraphens dargestellt. Auch wurde unter derselben Anströmung ein Konturgraph für die sich ergebenden Druckbeiwerte im Umgebungsfeld des Profils generiert. \\
...
17 & -0.5 & 2.02 \\ 
    ...
    \end{tabular}
    \label{tab:naca}
    %\caption{Ausgewählte Ausgewählte Ergebnisse des NACA-0012-Profils}
\end{table}
\end{minipage}
\hfill
\begin{minipage}{0.45\textwidth}
...
17 & -0.51 & 2.09 \\ 
    ...
    \end{tabular}
    \label{tab:nlf}
    %\caption{Ausgewählte Ergebnisse des NASA SC(2)-0614-Profils}
\end{table}
\end{minipage}
\hfill
\begin{minipage}{0.45\textwidth}
...
\begin{figure}[]
  ...
  \end{subfigure}
  \caption{Vergleich der \subref{fig:joukowskitrans}) Joukowski-Transformation mit der \subref{fig:karmantrefftztrans}) Kármán-Trefftz-Transformation. Für beide Transformationen wurde für die Verschiebung des Mittelpunkts des $\zeta $-Kreises $\mu_x = -0.3, \mu_y =0.4$ gewählt; für die Kármán-Trefftz-Transformation wurde $n=1.9$ gesetzt.\label{fig:joukar}}
\end{figure}
