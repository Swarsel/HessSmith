...
def make_panels(x, y, dir=False, flip=False, reverse=False):
    ...
    if dir:
        panels = np.array([Panel(x[n - i], y[n - i], x[n - i - 1], y[n - i - 1], i, flip) for i in range(n)])
    if not dir:
        panels = np.array([Panel(x[i], y[i], x[i +1], y[i +1], i, flip) for i in range(n)])
    return panels
